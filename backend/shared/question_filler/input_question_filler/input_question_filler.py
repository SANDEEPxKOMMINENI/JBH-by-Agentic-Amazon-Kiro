...
class InputQuestionFiller(QuestionFillerBasic):
    def __init__(
        self,
        config_reader,
        question,
        ...
        self.env = Environment(
            loader=FileSystemLoader("objects/question_filler/input_question_filler")
        )
        self.cached_faq = {}  # Initialize cached_faq

    def fill_value(self, value: Answer = Answer("", "", False)) -> str:
        error_message = ""
        element = self.question.locator("input")
        element_count = element.count()
        if element_count == 1 and not isinstance(value.answer, dict):
            first_element = element.first
            first_count = first_element.count()
            if first_count > 1:
                logger.warning("Found more than 1 input element, using the first one")
                element = first_element
            try:
                # Use browser operator's op method for consistent speed control
                if self.browser_operator:
                    self.browser_operator.op(element.fill, value=value.answer)
                else:
                    # Fallback to direct operation if no browser operator
                    element.fill(value.answer)
            except Exception as e:
                logger.error(
                    "Error filling input question: %s, value: %s, error: %s",
                    self.question_text,
                    value,
                    e,
                )
            ...
            logger.error(
                "No input element found for question: %s, value: %s",
                self.question_text,
                value,
            )
    ...
    def fill_value_with_ai_retry(self, value: Answer, num_retries: int = 3) -> Answer:
        ...
        for _ in range(num_retries):
            ...
            # blur the input
            if self.browser_operator:
                self.browser_operator.op(self.question.locator("input").first.blur)
            else:
                # Fallback to direct operation if no browser operator
                self.question.locator("input").first.blur()
    ...
    def fill_input_by_ai(self, retry_errors: list = []) -> Answer:
        answer = self.generate_text_answer(retry_errors)
        return answer
