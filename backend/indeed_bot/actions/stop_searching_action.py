#!/usr/bin/env python3
"""
Stop Searching Action for Indeed Bot
"""
...
class StopSearchingAction(BaseAction):
    """Action to stop the Indeed searching process"""
    ...
    def execute(self) -> Dict[str, Any]:
        """
        Stop the Indeed searching process, close browser, and eliminate bot
        """
        try:
            self.logger.info(f"Stopping Indeed bot {self.bot.bot_id}")
            ...
            self.logger.info(f"Indeed bot {self.bot.bot_id} stopped and eliminated")

            return {
                "success": True,
                "message": "Indeed bot stopped and browser eliminated",
                "status": "stopped",
            }

        except Exception as e:
            self.logger.error(f"Failed to stop Indeed bot: {e}")
