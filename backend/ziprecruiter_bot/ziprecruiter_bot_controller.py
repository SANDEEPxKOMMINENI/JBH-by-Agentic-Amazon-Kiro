#!/usr/bin/env python3
"""
ZipRecruiter Bot Controller for JobHuntr v2
@file purpose: Manage ZipRecruiter bot instances and WebSocket communication
"""
...
class ZipRecruiterBotController:
    """Controller to manage ZipRecruiter bot instances and activity polling"""

    def __init__(self):
        self.bots: Dict[str, ZipRecruiterBot] = {}
    ...
    def register_polling_session(self, workflow_run_id: str):
        """Register a session for activity polling"""
    ...
    def _send_activity_message(self, workflow_run_id: str, message: Dict[str, Any]):
        ...
        with self._message_lock:
            ...
            # Keep only the last 10000 messages per session
    ...
    def start_searching_controller(
        ...
        try:
            ...
            bot_id = f"ziprecruiter_bot_{workflow_run_id}_{uuid.uuid4().hex[:8]}"
            bot = ZipRecruiterBot(
                bot_id,
                user_id,
            ...
            logger.info(
                f"Created new ZipRecruiter bot for workflow run {workflow_run_id}: {bot_id}"
            )
    ...
    def stop_searching_controller(self, workflow_run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if already stopping this session
    ...
    def resume_searching_controller(self, workflow_run_id: str) -> Dict[str, Any]:
        """Resume searching for a session"""
        try:
            # Normalize workflow_run_id
            if workflow_run_id.startswith("user_session_"):
                parts = workflow_run_id.split("_", 2)
...
ziprecruiter_bot_controller = ZipRecruiterBotController()
